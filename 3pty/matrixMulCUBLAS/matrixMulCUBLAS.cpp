...
// Modifications have been made to this sample code for the purpose of varying
// the dimensions of the matrix multiplicands so that performance could be
// benchmarked in a remote computing cluster environment, caveats about
// not using the samples for benchmarking notwithstanding.
// - myantosca 2017-11-27
...
{
    fprintf(stderr, "Listing first %d Differences > %.6f...\n", iListLength, fListTol);
    int i,j,k;
    int error_count=0;
    ...
    for (j = 0; j < height; j++)
    {
        ...
        for (i = 0; i < width; i++)
        {
            ...
            if (fDiff > fListTol)
            {
                if (error_count < iListLength)
                {
		    fprintf(stderr, "    Loc(%d,%d)\tCPU=%.5f\tGPU=%.5f\tDiff=%.6f\n", i, j, data1[k], data2[k], fDiff);
                }
...
void initializeCUDA(int argc, char **argv, int &devID, int &m, int &q, int &n, sMatrixSize &matrix_size)
{
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "device"))
    {
        ...
        {
	    fprintf(stderr, "cudaSetDevice returned error code %d, line(%d)\n", error, __LINE__);
            exit(EXIT_FAILURE);
        }
    }
...
int matrixMultiply(int argc, char **argv, int devID, sMatrixSize &matrix_size)
{
    ...
    fprintf(stderr, "Computing result using CUBLAS...");
    ...
    //fprintf(stderr, "Computing result using host CPU...");
